...
if __name__ == '__main__':
    ...
    for p in ['DQ','JQ','JQ+COT','JQ+MC','JQ+COT+MC']:
        print(f'----:{p}')
        save_path = args.pred_path + f'_{p}' +'.csv'
        #save_path = 'try.csv'
        email_out_list = build_message(user_prompt, assistant_prompt, email_list, args,prompt_type = p,target= target, num_msg=1, is_enron_mail= is_enron_mail)
        save_prediction_to_csv(email_out_list,save_path)
